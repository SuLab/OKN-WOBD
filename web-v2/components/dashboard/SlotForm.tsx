...
const SLOT_LABELS: Record<string, { label: string; placeholder: string }> = {
  ...
  health_conditions: { label: "Disease (MONDO)", placeholder: "e.g. MONDO:0005015 or diabetes" },
  ...
  expand_mondo_descendants: { label: "Include MONDO descendants", placeholder: "" },
};
...
export function SlotForm({ template, values, onChange, disabled }: SlotFormProps) {
  ...
  const allOptional = template.optional_slots ?? [];
  // For genes-agreement template, show disease/MONDO slots in main area so disease input is visible
  const isGenesAgreement = template.id === "gene_expression_genes_agreement";
  const optionalVisibleFirst: string[] = isGenesAgreement
    ? ["health_conditions", "expand_mondo_descendants"].filter((s) => allOptional.includes(s))
    : [];
  const optional = isGenesAgreement
    ? allOptional.filter((s) => !optionalVisibleFirst.includes(s))
    : allOptional;
  ...
  const renderInput = (slotName: string) => {
    ...
    if (slotName === "expand_mondo_descendants") {
      const checked = raw === "true" || raw === "1" || (Array.isArray(raw) && (raw[0] === "true" || raw[0] === "1"));
      return (
        <div key={slotName} className="flex items-center gap-2">
          <input
            id={`slot-${template.id}-${slotName}`}
            type="checkbox"
            checked={checked}
            onChange={(e) => updateSlot(slotName, e.target.checked ? "true" : "")}
            disabled={disabled}
            className="h-4 w-4 rounded border-slate-300 dark:border-slate-600 text-niaid-header focus:ring-niaid-header"
            aria-describedby={undefined}
          />
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">Run query across disease subtypes (MONDO hierarchy)</span>
        </div>
      );
    }
  ...
  return (
    <div className="space-y-3">
      ...
      {optionalVisibleFirst.length > 0 && (
        <div className="space-y-3 pl-0 border-l-0">
          {optionalVisibleFirst.map(renderInput)}
        </div>
      )}
      {optional.length > 0 && (
        <>
