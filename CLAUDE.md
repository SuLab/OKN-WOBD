...
OKN-WOBD extracts biomedical dataset metadata from the NIAID Data Ecosystem Discovery Portal and converts it to RDF for loading into ProtoOKN/FRINK knowledge graphs. The project has three main products and several supporting packages:

1. **NDE RDF Pipeline** (`src/okn_wobd/cli.py`, `rdf_converter.py`) - Fetch data from NIAID API and convert to RDF (N-Triples and Turtle)
2. **GXA Pipeline** (`src/okn_wobd/gxa/`) - Download and convert Gene Expression Atlas experiments to Biolink-compatible RDF
3. **MCP Server** (`src/okn_wobd/mcp_server/`) - FastMCP server exposing analysis tools for AI assistants
4. **Clients** (`src/okn_wobd/clients/`) - Data source clients (SPARQL, NIAID, ARCHS4, CellxGene)
5. **Analysis** (`src/okn_wobd/analysis/`) - Gene-disease paths, gene neighborhoods, drug-disease analysis, visualization
6. **ChatGEO** (`src/okn_wobd/chatgeo/`) - Natural language differential expression analysis using ARCHS4
7. **FRINK** (`src/okn_wobd/frink/`) - FRINK knowledge graph integration (registry, context, NL-to-SPARQL)
8. **Questions** (`scripts/demos/questions/`) - Biological question investigations producing HTML reports
9. **Web UI** (`web-v2/`) - Next.js chat interface for SPARQL querying against FRINK federation

## Common Commands

```bash
# Install package in development mode (requires Python >= 3.11)
pip install -e ".[all]"              # Full install with all optional deps
pip install -e .                     # Core only (NDE + GXA pipelines)
pip install -e ".[analysis]"         # Add SPARQL analysis tools
pip install -e ".[chatgeo]"          # Add DE analysis + enrichment
...
# Run ChatGEO differential expression analysis
python -m okn_wobd.chatgeo.cli "psoriasis in skin tissue" --verbose
python -m okn_wobd.chatgeo.cli "psoriasis in skin tissue" --mode study-matched --verbose
python -m okn_wobd.chatgeo.cli "psoriasis in skin tissue" --mode pooled --verbose
...
Import: `from okn_wobd.clients import SPARQLClient, NIAIDClient, ARCHS4Client, CellxGeneClient`

### Analysis (`src/okn_wobd/analysis/`)
...
Import: `from okn_wobd.analysis import GeneDiseasePathFinder, GeneNeighborhoodQuery, PlotlyVisualizer`

### ChatGEO (`src/okn_wobd/chatgeo/`)
...
Import: `from okn_wobd.frink import FrinkContext, FrinkNL2SPARQL, FrinkRegistryClient`
...
FastMCP server wrapping `okn_wobd.analysis` and `okn_wobd.chatgeo`. Supports stdio (local), streamable-http, and SSE transports. Config templates in `config/mcp-dev.json` and `config/mcp-remote.json`.

- `server.py` - Server setup, transport selection, logging. Redirects stdout→stderr (MCP uses stdout for JSON-RPC).
- `tools_analysis.py` - SPARQL-based tools: `gene_disease_paths`, `gene_neighborhood`
...
Biological question investigations. Each module answers a specific question using the installed packages and produces an interactive HTML report:
...
### ARCHS4 SQLite Index
`okn_wobd.clients.archs4_index` builds a one-time SQLite index of ~1.05M ARCHS4 samples (~15s build, ~1.4GB). Stored as `.metadata.db` alongside HDF5 files. Provides ~60,000x speedup for metadata queries vs direct HDF5 access. Transparent fallback to HDF5 on index errors.
...
chatgeo = ["numpy", "scipy", "pydeseq2", "gprofiler-official", "h5py"]
cellxgene = ["cellxgene-census"]
viz = ["plotly"]
all = ["okn-wobd[chatgeo,cellxgene,viz]"]
```
...
- **Wikidata**: https://query.wikidata.org/sparql - Gene/disease lookups
