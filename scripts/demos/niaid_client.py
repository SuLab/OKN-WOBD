    ...
    def session(self) -> requests.Session:
        ...
        if self._session is None:
            ...
            self._session.headers.update({
                "User-Agent": "OKN-WOBD/1.0 (+https://github.com/SuLab/OKN-WOBD)"
            })
...
def demo():
    """Demonstrate the NIAID client capabilities."""
    print("=" * 70)
    print("NIAID Data Ecosystem Client Demo")
    print("=" * 70)

    client = NIAIDClient()

    # Demo 1: Simple search
    print("\n1. Simple Search: 'vaccine'")
    print("-" * 40)
    result = client.search("vaccine", size=3)
    print(f"Total found: {result.total}")
    for hit in result:
        print(f"  - {hit.get('name', 'Untitled')[:60]}")

    # Demo 2: Search by disease
    print("\n2. Search by Disease: 'influenza'")
    print("-" * 40)
    ...
    # Demo 4: Search by species
    print("\n4. Search by Species: Mouse (10090)")
    print("-" * 40)
    result = client.search_by_species("10090", keywords="vaccine", size=3)
    print(f"Total found: {result.total}")
    for hit in result:
        print(f"  - {hit.get('name', 'Untitled')[:60]}")

    # Demo 5: Filter by catalog
    print("\n5. Filter by Catalog: ImmPort")
    print("-" * 40)
    ...
    print("\n6. Facet Analysis")
    print("-" * 40)
    result = client.search("vaccine", facet_size=5)
    catalogs = result.get_facet_values("includedInDataCatalog.name")
    print("Top data sources for 'vaccine':")
    for facet in catalogs[:5]:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        result = client.search(query, size=10)
        print(f"Total found: {result.total}\n")

        for i, hit in enumerate(result, 1):
            print(f"--- Result {i} ---")
            print(client.format_dataset(hit))
            print()
    else:
        demo()
