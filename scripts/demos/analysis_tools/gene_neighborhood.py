...
Usage:
    ...
    python -m analysis_tools.gene_neighborhood --symbol CD19

    # By NCBI Gene ID
    python -m analysis_tools.gene_neighborhood --ncbi 930

    # With custom limits
    python -m analysis_tools.gene_neighborhood CD19 --limit 20
    python -m analysis_tools.gene_neighborhood CD19 --spoke-limit 15 --wikidata-limit 25

    # Output formats
    python -m analysis_tools.gene_neighborhood CD19 --format json
    python -m analysis_tools.gene_neighborhood CD19 --format table

    # Save to file
    python -m analysis_tools.gene_neighborhood CD19 -o cd19_neighborhood.json
...
Examples:
  ...
  %(prog)s CD19 --format json -o out.json
    ...
    parser.add_argument(
        "--html",
        help="Output interactive HTML network visualization",
    )
    ...
    if args.html:
        try:
            ...
            print(f"\nNetwork visualization saved to: {args.html}", file=sys.stderr)
        except ImportError:
            print("\nWarning: plotly not installed. Skipping visualization.", file=sys.stderr)
