...
Usage:
    from frink_registry_client import FrinkRegistryClient
    ...
    def session(self) -> requests.Session:
        ...
        if self._session is None:
            ...
            self._session.headers.update({
                "User-Agent": "FrinkContextBuilder/1.0 (+https://github.com/SuLab/OKN-WOBD)"
            })
...
def demo():
    """Demonstrate the FRINK registry client."""
    print("=" * 60)
    print("FRINK Registry Client Demo")
    print("=" * 60)
    ...
    registry = client.fetch_registry()
    print(f"   Found {len(registry)} knowledge graphs:")
    for entry in registry[:10]:
        print(f"   - {entry['shortname']}: {entry['title'][:50]}...")
    if len(registry) > 10:
        print(f"   ... and {len(registry) - 10} more")

    # Fetch schema for one graph
    print("\n2. Fetching schema for 'ubergraph'...")
    schema = client.fetch_schema_stats("ubergraph")
    if schema:
        print(f"   Classes: {len(schema.classes)}")
        print(f"   Properties: {len(schema.properties)}")
        print(f"   Prefixes: {len(schema.prefixes)}")
        if schema.classes:
            print(f"   Top classes by count:")
            for cls in sorted(schema.classes, key=lambda c: c.count, reverse=True)[:5]:
